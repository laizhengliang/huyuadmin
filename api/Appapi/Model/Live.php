...
class Model_Live extends PhalApi_Model_NotORM {
    ...
    public function getLiveInfo($liveuid){
        ...
        $info=DI()->notorm->users_live
					->select("uid,title,city,stream,pull,thumb,isvideo,type,type_val,game_action,goodnum,anyway,shopimg,shoplink")
					->where('uid=? and islive=1',$liveuid)
					->fetchOne();
        if($info){
            $configpri=getConfigPri();
            
            $nums=DI()->redis->zSize('user_'.$info['stream']);
			...
			$info['game']=getGame($info['game_action']);
