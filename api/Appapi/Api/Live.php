...
class Api_Live extends PhalApi_Api {
	...
	public function getRules() {
		return array(
			'createRoom' => array(
                ...
                'shoplink' => array('name' => 'shoplink', 'type' => 'string', 'default'=>'', 'desc' => '商品链接'),
			),
			'changeLive' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
				'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
	...
	public function createRoom() {
		...
		$shoplink = $this->shoplink?$this->shoplink:'';
		$shopimg='';
		if( $type==1 && $type_val=='' ){
			$rs['code']=1002;
			$rs['msg']='密码不能为空';
		...
		if($_FILES){
          ...
          if(!empty($_FILES["shopimg"])){
            
          if ($_FILES["shopimg"]["error"] > 0) {
				$rs['code'] = 1003;
				$rs['msg'] = T('failed to upload file with error: {error}', array('error' => $_FILES['shopimg']['error']));
				DI()->logger->debug('failed to upload file with error: ' . $_FILES['shopimg']['error']);
				return $rs;
			}
			
			if(!checkExt($_FILES["shopimg"]['name'])){
				$rs['code']=1004;
				$rs['msg']='图片仅能上传 jpg,png,jpeg';
				return $rs;
			}
			...
			if($uptype==1){
				//七牛
				$url = DI()->qiniu->uploadFile($_FILES['shopimg']['tmp_name']);

				if (!empty($url)) {
					$shopimg=  $url.'?imageView2/2/w/600/h/600'; //600 X 600
				}
			}else if($uptype==2){
				//本地上传
				//设置上传路径 设置方法参考3.2
			   ...
			   // DI()->ucloud->set('file_name', $this->uid);
				...
				//上传表单名
				$res = DI()->ucloud->upfile($_FILES['shopimg']);
			...
			@unlink($_FILES['shopimg']['tmp_name']);
	...
	public function enterRoom() {
      ...
      //var_dump($liveinfo);die;
      	$info['shopdetail'] = ['shopimg'=>$liveinfo['shopimg'],'shoplink'=>$liveinfo['shoplink']];
