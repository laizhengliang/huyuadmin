...
class RegController extends HomebaseController {
	...
	function index(){
        ...
        if($agentid){
            $where['user_type']=1;
            $where['user_activation_key']=$agentid;
            $agentuid=M('users')->where($where)->getField('id');
            if(!$agentuid){
	...
	public function wxLoginCallback(){
		...
		if($code){
			...
			$userinfo=$User->field("id")->where("openid!=''")->where($where)->find();
			...
			if(empty($userinfo)){
				if($openid!=""){
                    ...
                    if($agentid!=''){
                        $where2['user_type']=1;
                        $where2['user_activation_key']=$agentid;
                        $agentuid=M('users')->where($where2)->getField('id');
                        if(!$agentuid){
                            $this->assign("reason",'邀请码不正确');
                            $this->display(':error');
                            exit;
